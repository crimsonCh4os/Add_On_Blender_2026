...
class ANALI_OT_G1ChangeWindow(bpy.types.Operator):
    ...
    bl_label = "Change G1 Window"
    bl_description = "Move to the previous or next G1 data window and redraw the graph"
    ...
    def execute(self, context):
        scn = context.scene
        size = max(1, int(getattr(scn, 'anali_g1_window_size', 20)))
        ...
        if self.direction == 'PREV':
            ...
            if old_global:
                ...
                scn.anali_g1_window_start = old_start + size
...
class ANALI_OT_VisualizeTable(bpy.types.Operator):
    ...
    bl_label = "Visualize Data Table"
    bl_description = "Create a readable 3D data table from the selected CSV files and metrics"
...
class ANALI_OT_ClearTables(bpy.types.Operator):
    ...
    bl_label = "Clear Data Tables"
    bl_description = "Remove the 3D data tables generated by the add-on"
...
class ANALI_OT_TableChangePage(bpy.types.Operator):
    ...
    bl_label = "Change Table Page"
    bl_description = "Move to another table page and regenerate the 3D table"
