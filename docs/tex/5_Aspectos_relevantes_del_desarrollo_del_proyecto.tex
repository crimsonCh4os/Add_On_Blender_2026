...
La lectura del CSV incluye comprobaciones de consistencia para evitar que valores incompletos, no numéricos o no finitos alteren los resultados. Esta validación es importante porque las métricas calculadas dependen directamente de la calidad de los datos registrados~\cite{data_analysis}.
...
Esta separación permite distinguir entre métricas derivadas del registro de sesión y métricas calculadas directamente sobre la geometría seleccionada. Las primeras describen la evolución temporal del proceso; las segundas analizan propiedades técnicas del modelo en el momento de la evaluación~\cite{botsch2010polygon,cignoni1998metro}.
