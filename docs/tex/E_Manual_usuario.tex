...
Este manual describe el uso práctico de la \textit{suite} formada por dos add-ons para Blender. La organización del contenido sigue un enfoque guiado por tareas, habitual en documentación de usuario y usabilidad, donde cada acción se acompaña de explicación y comprobación visual~\cite{nielsen1994usability}. El primer add-on, \textit{Data Logger 3D}, permite registrar una sesión de modelado y exportar un CSV con los datos capturados. El segundo add-on permite leer esos CSV, calcular métricas estadísticas A1--A17, obtener métricas geométricas y UV, y generar visualizaciones dentro de Blender.
...
En la Figura~\ref{fig:e-install-addon} se localiza el botón \textit{Install}, que abre el explorador de archivos. Desde esa ventana se selecciona primero el archivo \texttt{DeteccionDatosV6.py} y, posteriormente, el archivo ZIP del add-on de análisis. Tras seleccionar cada archivo, Blender añade el complemento al listado de add-ons disponibles.
...
\subsection{Vídeo demostrativo del funcionamiento}
\label{subsec:video_demostrativo}

Como material complementario de validación, se incluye un vídeo demostrativo del funcionamiento completo del sistema. En dicho vídeo se muestra la instalación o activación de los add-ons, el inicio de una sesión de captura, la realización de operaciones de modelado en Blender, la exportación del fichero CSV, la carga del CSV en el módulo de análisis, el cálculo de métricas y la generación de visualizaciones.

Este vídeo permite comprobar el comportamiento general del software sin depender exclusivamente de la instalación local del tribunal, lo cual resulta especialmente útil al tratarse de add-ons dependientes del entorno Blender, de su API interna y de determinadas versiones de Python.

La Figura~\ref{fig:e-panel-analisis-csv} muestra la pestaña inicial de análisis CSV. Desde esta vista se seleccionan archivos o carpetas, se lanzan los cálculos estadísticos y se revisan los resultados asociados a las métricas A1--A17.
...
La Tabla~\ref{tab:e-interpretacion-a} resume cómo debe leerse cada bloque de métricas. Esta tabla no sustituye a los resultados generados por el add-on, sino que actúa como guía para interpretar los valores mostrados en pantalla. Cuando se obtengan capturas definitivas de los resultados A1--A17, podrán incorporarse como material complementario.
%\begin{figure}[H]
%    \centering
...
La Figura~\ref{fig:e-resultados-geometricos} muestra el panel de resultados de métricas geométricas y UV. Si algún valor no aparece, debe comprobarse que el objeto tiene malla válida, que existe una capa UV activa y que se ha seleccionado referencia cuando la métrica lo necesita.
