...
El formato CSV (\textit{Commas-Separated Values}) ha servido como la base para registrar el registro secuencial de las interacciones~\cite{data_analysis}.
...
La biblioteca \texttt{bmesh} se ha utilizado para acceder a la representación editable de las mallas de Blender. Su uso resulta especialmente relevante en operaciones de análisis topológico, recuento de caras, cálculo de islas UV y consulta de aristas compartidas entre caras. Esta capa proporciona un control más preciso que la estructura \texttt{Mesh} evaluada cuando el usuario trabaja en modo edición~\cite{blenderapi,botsch2010polygon}.
