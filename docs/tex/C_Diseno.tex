...
\begin{longtable}{p{3.4cm}p{4.2cm}p{15.5cm}}
...
\caption[]{Campos completos del CSV utilizado como formato intermedio entre captura y análisis (continuación).}\\
\toprule
\textbf{Grupo} & \textbf{Campo} & \textbf{Descripción de diseño} \\
\midrule
\endhead
...
Identificación & \texttt{LoggerVersion} & Versión del \textit{\textit{add-on}} \textit{Data Logger 3D} que generó el archivo CSV. Permite relacionar los datos con la versión concreta del software. \\
...
\bottomrule
\end{tabularx}
\caption{Variables derivadas principales calculadas por el \textit{add-on} de análisis.}
...
La arquitectura se estructura en dos \textit{add-ons} conectados mediante un CSV intermedio. Esta división responde a criterios clásicos de modularidad y separación de responsabilidades, donde cada componente encapsula una función concreta del sistema~\cite{parnas1972criteria,gamma1994}. Esta separación permite que la captura sea ligera y que el análisis se ejecute bajo demanda. El primer \textit{add-on} no necesita conocer las métricas finales; el segundo \textit{add-on} no necesita intervenir durante la sesión de modelado. La Figura~\ref{fig:c-flujo-arquitectura} resume esta relación entre captura, exportación, análisis y visualización.
...
Como se observa en la Figura~\ref{fig:c-flujo-arquitectura}, el CSV actúa como punto de unión entre los dos \textit{add-ons}. Esta decisión evita acoplar directamente el proceso de captura con el cálculo de métricas y permite revisar los datos de forma independiente antes del análisis.
...
\textit{add-on} de análisis & Lee, valida, calcula métricas y genera visualizaciones sin modificar los datos originales. \\
...
\section{Diseño del primer \textit{add-on}: Data Logger 3D}
...
\bottomrule
\end{tabularx}
\caption{Diseño interno del \textit{add-on} Data Logger 3D.}
...
\midrule
Detenido & \textit{add-on} cargado sin captura activa & No se escriben registros y el panel solo muestra controles de inicio o consentimiento. \\
\midrule
...
\section{Diseño del segundo \textit{add-on}: análisis y visualización}
...
La Tabla~\ref{tab:c-flujo-modulos-analisis} resume esta transformación desde las entradas de usuario hasta las salidas generadas por el \textit{add-on}. De este modo se documenta qué recibe cada módulo lógico y qué resultado produce dentro del diseño general.
...
\bottomrule
\end{tabularx}
\caption{Transformación de entradas y salidas en el diseño del \textit{add-on} de análisis.}
...
\begin{enumerate}
    \item Se abre Blender y se activa el \textit{add-on} \textit{Data Logger 3D}.
    ...
    \item El \textit{add-on} de análisis lee uno o varios CSV y valida sus datos.
    \item Se calculan métricas A1--A16, métricas geométricas y métricas UV.
    \item Los resultados se presentan en paneles y visualizaciones dentro de Blender.
\end{enumerate}

\section{Diseño de métricas A1--A16}
\label{sec:c-diseno-a1-A16}
...
A16 & Mesh evolution & Strategy & Resume variaciones de objeto en \textit{Object Mode}. \\
\bottomrule
\end{tabularx}
\caption{Diseño de presentación de las métricas A1--A16.}
\label{tab:c-diseno-a1-A16}
\end{table}
...
Durante el diseño de la \textit{suite} se tomaron varias decisiones orientadas a mejorar la modularidad, la mantenibilidad y el rendimiento del sistema. Estas decisiones afectan tanto a la separación entre captura y análisis como al formato de intercambio de datos, la forma de registrar cambios, el acceso a la información interna de Blender y la organización de la interfaz.

La Tabla~\ref{tab:c-decisiones-diseno} resume las decisiones de diseño más relevantes y justifica su adopción dentro del proyecto. En conjunto, estas decisiones permiten que el sistema mantenga una captura ligera durante la sesión de modelado y desplace las operaciones más costosas a una fase posterior de análisis.
...
\midrule
Separación en dos \textit{add-ons} & Evita que el análisis pesado interfiera con el modelado y permite evolucionar cada parte por separado. \\
