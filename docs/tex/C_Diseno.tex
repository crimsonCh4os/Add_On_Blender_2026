...
\midrule
Identificación & \texttt{USER\_ID} & Identificador seudonimizado de la sesión o equipo. No almacena nombre real del usuario. \\
...
La arquitectura se estructura en dos add-ons conectados por CSV. Esta separación permite que la captura sea ligera y que el análisis se ejecute bajo demanda. El primer add-on no necesita conocer las métricas finales; el segundo add-on no necesita intervenir durante la sesión de modelado.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{diagramas/flujo_suite.png}
    \caption{Flujo general de la suite: captura, exportación CSV, análisis de métricas y visualización en Blender.}
...
El primer add-on se encarga de registrar los cambios relevantes que se producen durante la sesión de modelado. Su prioridad es registrar información útil sin ralentizar la interacción del usuario. Para ello, se organizan varios subsistemas que comparten un estado global mínimo.
...
El segundo add-on se diseña como una capa de procesamiento posterior. La lectura CSV, la validación, el cálculo estadístico, el cálculo geométrico y la interfaz se mantienen separados para reducir dependencias cruzadas y facilitar el mantenimiento.
...
Cálculo UV & Calcula área UV, islas, estiramiento y densidad de texeles(Resolución de la textura). \\
...
El procedimiento general se divide en captura, persistencia, análisis y visualización. Esta secuencia permite reproducir el proceso y facilita la depuración cuando una métrica presenta valores inesperados.
...
Las métricas geométricas y UV se calculan directamente sobre objetos de Blender. Su diseño se separa de las métricas CSV porque dependen del estado de la malla, de capas UV y, en algunos casos, de un objeto de referencia.
...
\texttt{UV texel density} & Relación entre área geométrica y área UV. \\
...
Uso de KDTree & Se incorpora en las métricas geométricas que requieren búsquedas de proximidad entre puntos, especialmente en comparaciones entre mallas. Su uso permite reducir el coste de cálculo frente a comparaciones exhaustivas y mejora el rendimiento en modelos con mayor número de vértices. \\
...
\bottomrule
\end{tabularx}
\caption{Decisiones de diseño relevantes de la suite.}
