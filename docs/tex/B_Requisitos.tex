...
Este apéndice recoge la especificación de requisitos del sistema desarrollado. La solución se organiza como una suite de dos add-ons para Blender: \textit{Data Logger 3D}, orientado a la monitorización y captura de datos durante sesiones de modelado 3D, y un segundo add-on de análisis y visualización, encargado de procesar los ficheros CSV generados, calcular métricas estadísticas, geométricas y UV, y presentar los resultados dentro del propio entorno de Blender.

La especificación se plantea con un nivel de detalle suficiente para justificar las decisiones de diseño y facilitar la trazabilidad entre objetivos, funcionalidades y módulos implementados. No se pretende reproducir el código fuente ni describir exhaustivamente cada función, sino delimitar qué debe hacer el sistema, bajo qué restricciones debe funcionar y qué garantías mínimas debe ofrecer en términos de rendimiento, robustez, usabilidad y tratamiento de datos.
...
% [Insertar figura: diagrama de casos de uso del sistema]
\begin{figure}[H]
    \centering
