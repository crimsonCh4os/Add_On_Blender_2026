...
\subsection{Instalación de dependencias en el Python de Blender}
\label{subsec:d-instalacion-dependencias-dev}

Las dependencias externas deben instalarse en el Python integrado de Blender. En Windows, una forma habitual es ejecutar el Python de Blender desde la carpeta de instalación. La ruta exacta puede variar según la versión instalada:

\begin{verbatim}
"C:\\Program Files\\Blender Foundation\\
Blender 4.x\\4.x\\python\\bin\\python.exe"
 -m ensurepip
"C:\\Program Files\\Blender Foundation\\
Blender 4.x\\4.x\\python\\bin\\python.exe"
 -m pip install numpy matplotlib
\end{verbatim}

En Linux o macOS se debe usar de forma equivalente el ejecutable de Python incluido en la instalación de Blender. Si NumPy o Matplotlib ya están disponibles en la distribución utilizada, no es necesario reinstalarlos. En cualquier caso, las dependencias deben comprobarse desde Blender, ya que el Python del sistema y el Python de Blender pueden tener paquetes distintos.
...
\hline
P-07 & Cálculo de métricas A1--A17 & Manual & El sistema calcula y muestra las métricas estadísticas & Captura \\
\hline
...
\subsection{Pruebas funcionales en Blender}
\label{subsec:d-pruebas-funcionales-blender}

En el add-on \texttt{Data\_Logger\_3D.py} se comprobaron las operaciones principales de inicio y parada de la captura, aceptación del consentimiento, restauración del CSV desde el archivo \texttt{.blend}, exportación del CSV, captura de cambios de modo, detección de operadores relevantes, detección de cambios UV y persistencia de los datos en bloques de texto internos.

En el add-on de análisis se comprobaron la carga de archivos CSV, la validación de registros, el cálculo de métricas A1--A17, el cálculo de métricas geométricas y UV, la generación de visualizaciones y la actualización de la interfaz. Estas pruebas se realizaron dentro de Blender, ya que dependen del contexto gráfico, de objetos de tipo malla, de paneles, operadores y propiedades registradas mediante la API \texttt{bpy}.

\begin{table}[H]
\centering
\small
\caption{Resumen de pruebas de compatibilidad en distintos equipos}
\label{tab:compatibilidad_equipos}
\begin{tabularx}{\textwidth}{|X|X|c|X|X|}
\hline
\textbf{Equipo} & \textbf{Sistema operativo} & \textbf{Versión de Blender} & \textbf{Resultado} & \textbf{Observaciones} \\
\hline
Equipo principal de desarrollo & Windows / Linux / macOS & Blender 4.x & Correcto & Entorno usado durante el desarrollo \\
\hline
Equipo externo 1 & Windows & Blender 4.x & Correcto & Instalación realizada siguiendo las dependencias indicadas en el manual de usuario \\
