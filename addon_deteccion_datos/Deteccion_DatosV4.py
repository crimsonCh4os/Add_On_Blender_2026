...
def logger_timer():
    ...
    if row:
        ...
        # ❌ NO actualizamos el TextBlock mientras corremos el logger
        # import_csv_to_blend()  # comentado hasta stop_logger()
...
class DATA_LOGGER_OT_Start(bpy.types.Operator):
    bl_idname = "wm.start_logger"
    bl_label = "Start Logger"
...
class DATA_LOGGER_OT_Stop(bpy.types.Operator):
    bl_idname = "wm.stop_logger"
    bl_label = "Stop Logger"
...
def stop_logger():
    ...
    # ✅ Al detener, incrustamos el CSV temporal en el TextBlock
    import_csv_to_blend()
    print("Logger detenido y CSV actualizado en el .blend.")

def restore_csv_from_blend():
    """Carga el CSV incrustado en memoria, solo para referencia al abrir Blender."""
    ...
    # NO sobrescribimos TEMP_CSV_PATH aquí, solo podemos mostrarlo en consola si quieres
    print("CSV antiguo disponible en el .blend, listo para consultar.")
    return True
...
class DATA_LOGGER_PT_Panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        # --- Control del Logger ---
        layout.label(text="Control del Logger:")
        row = layout.row()
        row.operator("wm.start_logger", icon="PLAY")
        row.operator("wm.stop_logger", icon="PAUSE")
